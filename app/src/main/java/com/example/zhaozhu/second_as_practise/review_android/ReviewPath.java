...
public class ReviewPath {
    ...
    private void commonMethods() {
        ...
        mPath.setFillType(Path.FillType.WINDING);//表示设置填充类型
        ...
        mPath.close();//是关闭当前轮廓，如果起始点和终止点不重叠就会用一条直线连接
        ...
        mPath.offset(200, 300, mPath1);//表示将当前path平移（dx，dy）之后存储到dst中
    ...
    private void init01() {
        ...
        mPath1 = new Path();
        mPath2 = new Path();
        mPath1.addCircle(300, 800, 280, Path.Direction.CCW);
        mPath2.addRect(400, 900, 800, 1300, Path.Direction.CCW);
        //----Op,5种类型
        //--1
